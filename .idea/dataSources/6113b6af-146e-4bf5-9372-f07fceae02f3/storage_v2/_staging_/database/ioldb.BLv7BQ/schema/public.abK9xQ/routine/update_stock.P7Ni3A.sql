...
BEGIN
    ...
    if tg_table_name = 'approv' then
     ...
     if tg_table_name = 'transfert' then
        -- Mis a jour du stock de la magasin source
        update stock set count = (count - (NEW.quantite*stock_quantite))where article_id = new.article_id AND unite_id = primary_unite_id AND magasin_id = new.magasin_source_id;
